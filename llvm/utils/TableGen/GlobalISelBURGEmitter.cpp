...
public:
  Operator(OperatorNum Num, const CodeGenInstruction *Inst, unsigned Arity)
      : Kind(Op_Inst), Num(Num), Inst(Inst), RegClass(nullptr), Arity(Arity),
        IntValue(0) {
    Map.resize(Arity);
    Reps.resize(Arity);
    if (Arity > 0)
  ...
  Operator(OperatorNum Num, int IntValue)
      : Kind(Op_Value), Num(Num), Inst(nullptr), RegClass(nullptr), Arity(0),
        IntValue(IntValue) {}
...
public:
  ...
  void update(const Nonterminal *NT, const Rule *R, CostValue Cost) {
    ...
    if (Cost == CostValue::infinity())
      Items.erase(NT->getNum());
    else
      Items.insert_or_assign(NT->getNum(), Item{R->getNum(), Cost});
  }

  RuleNum getRuleWithMinimalCost() const {
    CostValue Cost = Cost.infinity();
    RuleNum R = ~0U;
    for (auto &[Nt, Data] : Items) {
      if (Data.Cost < Cost) {
        Cost = Data.Cost;
        R = Data.Num;
      }
...
public:
  ...
  void update(Nonterminal *NT, CostValue Cost) {
    ...
    if (Cost == CostValue::infinity())
      ...
      Items.insert_or_assign(NT->getNum(), Cost);
  }
...
class BURSTableGenerator {
  ...
  // and the flag false are returned.
  template <typename T>
...
Rule *BURSTableGenerator::createRule(Operator *Op, OperandList &&Operands,
  ...
  // that no other rule uses the nonterminal gpr. This drastically reduces the
  // number of nonterminals and rules, which speeds up the generation. As a side
  // effect, the number of different representer sets also drops, which
...
void BURSTableGenerator::run() {
  ...
  // TODO Using 0 as the error state seems convenient, but it also makes the
  //      mapping and transition tables larger. The interpretation of the error
  //      state is that there is matching pattern for the current instruction,
  //      which implies that the user-provided select() function needs to be
  //      called. However, even if we don't have a matching pattern for the
  //      instruction, the output operand still has a type and a register class.
  //      Thus, when the parent node is labelled, the state of the child is not
  //      the error state but a register class. This means, the error state is
  //      never used in a lookup of the mappging and transition tables, it can
  //      only occur as the result of a transition. As consequence, we could
  //      assign a high number to the error state, e.g. ~0U, and simply ignore
  //      it during the construction.
  // State 0 is the error state. This state would be added anyway during the
  // computation of the transistions. Adding it now makes sure it has the number
  // 0, which makes it easy to test for.
  State ErrorState(0);
  insert(States, std::move(ErrorState));
  // TODO Does the error state needs processing? I currently assume no.
  //      If this is wrong then the line above needs to be replaced with
  // WorkList.push(insert(States, std::move(ErrorState)).first);
  computeLeafStates();
...
void BURSTableGenerator::emit(raw_ostream &OS) {
  ...
  for (auto &S : States) {
    ...
    S.dump(OS);
    OS << "\n";
    RuleNum RId = S.getRuleWithMinimalCost();
    if (RId < Rules.size()) {
      Rule *R = Rules[RId];
      if (R->getPattern()) {
        OS << "    case " << S.getNum() << ":\n";
...
GlobalISelBURGEmitter::importPatternToMatch(BURSTableGenerator &BURS,
  ...
  Expected<Rule *> R =
      patternToRule(BURS, Src, CostValue::fromComplexity(Score));
  if (R)
    (*R)->setPattern(&Pat);
  return R;
}
