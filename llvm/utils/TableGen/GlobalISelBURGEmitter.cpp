...
public:
  ...
  RuleNum getRuleWithMinimalCost() {
    CostValue Cost = Cost.infinity();
    RuleNum R = ~0U;
    for (auto &[Nt, Data] : ItemSet) {
      if (Data.Cost < Cost) {
        Cost = Data.Cost;
        R = Data.Num;
      }
  ...
  void dump(raw_ostream &OS) {
    OS << "State " << Num << ":";
    for (auto &[NT, Data] : ItemSet) {
      OS << " [" << NT << ": { " << Data.Num << ", " << Data.Cost.getValue()
         << "} ]";
    }
  }
};
...
class BURSTableGenerator {
  ...
  public:
    ...
    void free(T *Ptr) {
      assert(Cached == nullptr && "Expected no cached element");
      Ptr->reset();
      Cached = Ptr;
    }
  };

  Allocator<State> StateAllocator;
  Allocator<ProjectedState> ProjectedStateAllocator;
  ...
  template <typename T>
  std::pair<T *, bool> insert(FoldingSet<T> &Set, T *Elem) {
    FoldingSetNodeID ID;
    Elem->Profile(ID);
    void *InsertPoint;
    if (T *Tmp = Set.FindNodeOrInsertPos(ID, InsertPoint))
      return std ::pair<T *, bool>(Tmp, false);
    Elem->setNum(Set.size());
    Set.InsertNode(Elem, InsertPoint);
...
void BURSTableGenerator::computeLeafStates() {
  ...
  for (Operator &Op : Operators) {
    if (Op.arity() != 0)
      continue;
    State *S = StateAllocator.allocate();
    ...
    auto [TmpS, Inserted] = insert(States, S);
    if (!Inserted) {
      StateAllocator.free(S);
      S = TmpS;
    } else
      WorkList.push(S);
    Op.setLeafState(S);
...
ProjectedState *BURSTableGenerator::project(Operator *Op, unsigned Idx,
  ...
  llvm::dbgs() << "project op " << Op->getNum() << " state " << S->getNum()
               << "\n";
  ProjectedState *P = ProjectedStateAllocator.allocate();
  ...
  // Record the state if not yet known.
  auto [TmpP, Inserted] = insert(ProjectedStates, P);
  if (!Inserted) {
    ProjectedStateAllocator.free(P);
    P = TmpP;
  }
  return P;
}
