...
static void eraseInstruction(Instruction *I) {
  for (auto *U : I->users()) {
    eraseInstruction(dyn_cast<Instruction>(U));
  }
...
bool M88kFFS::runOnBasicBlock(BasicBlock &BB) {
  ...
  for (BasicBlock::iterator I = BB.begin(); I != BB.end(); ++I) {
    ...
    if (match(&(*I),
      ...
      auto *USub =
          Builder.CreateIntrinsic(Intrinsic::usub_with_overflow,
                                  {Int32Ty, Int32Ty}, {Int32Zero, Input});
      ...
      EraseList.push_back(Cttz);
      Cttz = nullptr;

      // Replace with the following IR:

      // %usub = tail call {i32, i1} @llvm.usub.with.overflow.i32(i32 0, i32
      // noundef %x) %res = extractvalue {i32, i1} %usub, 0 %overbit =
      // extractvalue {i32, i1} %usub, 1 %over = zext i1 %overbit to i32 %and =
      // and i32 %res, %x %add = add i32 %and, %and %add2 = add i32 %add, %over
