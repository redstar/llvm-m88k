//===- llvm/CodeGen/GlobalISel/BURSInstructionSelect.cpp -------------------==//
//
...
#include "llvm/CodeGen/GlobalISel/BURSInstructionSelect.h"
#include "llvm/ADT/PostOrderIterator.h"
#include "llvm/ADT/ScopeExit.h"
#include "llvm/Analysis/LazyBlockFrequencyInfo.h"
#include "llvm/Analysis/ProfileSummaryInfo.h"
#include "llvm/CodeGen/GlobalISel/BURSInstructionSelector.h"
...
#define DEBUG_TYPE "burs-instruction-select"
...
static cl::opt<std::string>
    CoveragePrefix("burs-gisel-coverage-prefix",
...
char BURSInstructionSelect::ID = 0;
INITIALIZE_PASS_BEGIN(BURSInstructionSelect, DEBUG_TYPE,
...
INITIALIZE_PASS_END(BURSInstructionSelect, DEBUG_TYPE,
                    "Select target instructions out of generic instructions",
                    false, false)

BURSInstructionSelect::BURSInstructionSelect(CodeGenOptLevel OL)
...
BURSInstructionSelect::BURSInstructionSelect()
    : MachineFunctionPass(ID), OptLevel(CodeGenOptLevel::Default) {}

void BURSInstructionSelect::getAnalysisUsage(AnalysisUsage &AU) const {
...
bool BURSInstructionSelect::runOnMachineFunction(MachineFunction &MF) {
  ...
  BURSInstructionSelector *ISel =
      MF.getSubtarget().getBURSInstructionSelector();
  assert(ISel && "Cannot work without BURSInstructionSelector");
  ...
  for (MachineBasicBlock *MBB : post_order(&MF)) {
    ...
    // Label instructions in normal (bottom-up) order block order. We permit
    // erasing so have to resort to manually iterating and recognizing the end
    // case.
    bool ReachedEnd = false;
    for (auto MII = MBB->begin(), End = MBB->end(); !ReachedEnd;) {
      ...
      if (ISel->preISelLower(MI)) {
        // TODO Adjust MI;
      }
      ISel->label(MI);
    }

    // Reduce instructions in reverse block order. We permit erasing so have
